...
class AppDelegate: NSObject, NSApplicationDelegate{
    ...
    func applicationDidFinishLaunching(_ aNotification: Notification) {
        ...
        let openFilesItem = menu.addItem(withTitle: NSLocalizedString("openFilesInFinder", value: "Open files in finder", comment: ""), action: #selector(toggleOption(_:)), keyEquivalent: "")
        openFilesItem.state = Preferences.openFilesInFinder ? .on : .off
        openFilesItem.tag = .openFiles
        menu.addItem(NSMenuItem.separator())
    ...
    @objc func toggleOption(_ sender: NSMenuItem) {
       ...
       case .openFiles:
            Preferences.openFilesInFinder = shouldBeOn
        default:
